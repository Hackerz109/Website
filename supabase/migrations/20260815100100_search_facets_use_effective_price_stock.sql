-- Switches price_min/price_max/pass_price and pass_stock to read
-- effective_price_cents/effective_in_stock instead of price_cents/
-- stock+stock_unlimited — see search_effective_price_and_stock for why.
-- Everything else about this function (the faceted-counts pattern,
-- category/brand/rating logic) is unchanged.
...
AS $function$
  ...
  flagged AS MATERIALIZED (
    SELECT
      ...
      (p_min_price IS NULL OR b.effective_price_cents >= p_min_price) AND (p_max_price IS NULL OR b.effective_price_cents <= p_max_price) AS pass_price,
      (p_min_rating IS NULL OR COALESCE(b.rating_avg,0) >= p_min_rating) AS pass_rating,
      (p_in_stock_only IS NOT TRUE OR b.effective_in_stock) AS pass_stock
  ...
  price_bounds AS (
    SELECT MIN(effective_price_cents) AS min_price, MAX(effective_price_cents) AS max_price
  ...
  stock_counts AS (
    SELECT
      COUNT(*) FILTER (WHERE f.effective_in_stock) AS in_stock,
      COUNT(*) FILTER (WHERE NOT f.effective_in_stock) AS out_of_stock
    FROM flagged f
