 ...
 *
 * No 3D/canvas library — the site has none, so the geometry is done by
 * hand: every node's (angle, radius, y) is rotated + perspective-projected
 * in JS each frame. Two things matter for keeping this cheap:
 *
 *  1. Per-frame writes only ever touch `transform`/`opacity`/CSS vars,
 *     never `left`/`top` — those force a full layout recalc on every
 *     element, every frame, which is the classic way this kind of
 *     animation ends up janking the whole page. Nodes are anchored at
 *     the container's center via `left:50%; top:50%` ONCE, and moved
 *     from there purely with `translate()`, which the compositor can
 *     handle without re-running layout.
 *  2. An IntersectionObserver + visibilitychange listener stop the
 *     requestAnimationFrame loop entirely when the helix is scrolled out
 *     of view or the tab is backgrounded, instead of quietly spending
 *     CPU forever in the background.
 *
 ...
 * exactly where you left it. Every few seconds one product eases outward
 * and a label names it, then it settles back into the spiral.
 */
...
const HALF_W = 150;
const HALF_H = 210;
// Curve resolution for the connecting strands. 36 is plenty smooth at this
// display size and halves the per-frame trig/path cost versus a denser
// curve — the glow filter softens the joints further anyway.
const STRAND_SAMPLES = 36;
...
function pathFromStrand(strand: StrandPoint[], rotationY: number, pitch: number): string {
  ...
  for (let i = 0; i < strand.length; i++) {
    ...
    parts.push((i === 0 ? "M " : "L ") + lx.toFixed(1) + " " + ly.toFixed(1));
  }
  return parts.join(" ");
}
...
export function HeroHelix() {
  ...
  useEffect(() => {
    ...
    // Anchor every node at the container's own center via percentage
    // left/top — a single one-off write, not a per-frame one. Everything
    // after this happens purely through `transform`.
    nodeRefs.current.forEach((el) => {
      el.style.left = "50%";
      el.style.top = "50%";
    });

    // Cached bounding rect — refreshed on resize/scroll instead of on
    // every animation frame, so the hot loop never calls
    // getBoundingClientRect() itself.
    ...
    function frame(now: number) {
      ...
      const worldToPx = (rect.width || 1) / (HALF_W * 2);
      ...
      [...projections]
        ...
        .forEach((item, rank) => {
          ...
          if (!el) return;
          const pxX = (item.lx * worldToPx).toFixed(1);
          const pxY = (item.ly * worldToPx).toFixed(1);
          // transform only — no layout triggered by this write.
          el.style.transform = `translate(-50%, -50%) translate(${pxX}px, ${pxY}px)`;
      ...
      // Strands + pulses redraw at ~30fps instead of 60 — the rotation is
      // slow enough that the extra frames aren't visible, and this is the
      // most expensive part of the loop (path geometry + blur filter).
    ...
    // Only animate while the helix is actually on screen and the tab is
    // active — otherwise this would happily spend CPU forever, anywhere
    // else on the site, or while the tab sits in the background.
