...
import { Store, Truck, MapPin, ImagePlus, X, PackageSearch } from "lucide-react";
...
export function OrderTrackingPanel({ orderId, userId }: OrderTrackingPanelProps) {
  ...
  const { data: returns } = useQuery({
    queryKey: ["order-returns", orderId, userId],
    queryFn: async () => {
      const all = await fetchMyReturns(userId);
    ...
    return (
      <div className="p-6 text-center">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          {(orderError as { message?: string })?.message ?? "Please try again."}
        </p>
      </div>
    );
  }
      ...
      {canRequestReturn && (
        <div className="rounded-xl border p-4">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-semibold">Need to return something?</h2>
            <Dialog open={returnOpen} onOpenChange={setReturnOpen}>
              ...
              <DialogContent className="max-h-[85vh] overflow-y-auto">
                <ReturnRequestForm
                  ...
                  onDone={() => {
                    setReturnOpen(false);
                    qc.invalidateQueries({ queryKey: ["order-returns", orderId, userId] });
                    qc.invalidateQueries({ queryKey: ["order-detail", orderId] });
                  }}
                />
              </DialogContent>
