...
- Address and map location are fully independent: typing an address never moves the map pin, and dropping a pin (via "Use my current location", tap, or drag) never overwrites the typed address fields. A full address (line 1/city/state/6-digit pincode) is always required to place a delivery order, regardless of whether a pin was ever set. Dropping a pin is optional — the UI frames it as "makes it easy for us to find your exact spot" — but when the shopper skips it, the typed address is silently geocoded server-side (Nominatim) in the background purely to run the same delivery-zone eligibility/charge check a manual pin would trigger; that guess is never shown as a marker and a real pin always takes priority over it the moment one exists.
...
- **Nominatim (OpenStreetMap), no API key** — all reverse/forward geocoding and pincode lookups happen server-side in `src/lib/geocode.server.ts` (Nominatim for address↔coordinates, India Post's pincode API for pincode→city/state), never from the browser directly. A serialized ~1 req/sec throttle plus a short in-memory cache keep it inside Nominatim's public-instance usage policy; set `NOMINATIM_CONTACT` in the deployment env to a real contact so OSM can identify this traffic if it ever needs to. If checkout volume grows enough that shared-IP throttling becomes a real problem, this is the one file that would need to move to a paid/self-hosted provider.
